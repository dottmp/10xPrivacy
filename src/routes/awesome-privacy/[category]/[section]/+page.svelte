...
<main class="mx-auto max-w-4xl px-4 py-10">
	...
	<header class="mb-4">
		...
		<p class="mt-0.5 text-sm text-base-content/60">
			{data.section.services.length} privacy-respecting tools
		</p>
	</header>
	...
	{#if data.section.intro}
		<div class="mb-6 bg-base-100 p-4 text-sm leading-relaxed text-base-content/70 shadow-sm">
			{data.section.intro}
		</div>
	{/if}
		...
		{#each data.section.services as service (service.name)}
			...
			<li>
				<a
					...
					class="group flex gap-4 bg-base-100 p-5 shadow-sm transition-shadow hover:shadow-md"
				>
					<!-- Icon -->
					<div class="mt-0.5 shrink-0">
						{#if service.icon}
							<img src={service.icon} alt={service.name} class="h-8 w-8 object-contain" />
						{:else}
							<div
					...
					<div class="min-w-0">
						<div class="flex flex-wrap items-center gap-2">
							<h2 class="font-semibold text-base-content group-hover:text-primary">
								{service.name}
							</h2>
						...
						<p class="mt-1 line-clamp-2 text-xs text-base-content/60">{service.description}</p>
					</div>
	...
	{#if data.section.wordOfWarning}
		<div
			class="mt-8 flex gap-3 border border-warning/30 bg-warning/10 p-4 text-sm text-base-content/80"
		>
			<i class="nf nf-fa-exclamation_triangle mt-0.5 shrink-0 text-warning"></i>
			<p class="leading-relaxed">{data.section.wordOfWarning}</p>
		</div>
	{/if}

	<div class="mt-8">
		<a href={resolve(`/awesome-privacy/${data.categorySlug}`)} class="btn btn-ghost">
